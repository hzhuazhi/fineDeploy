...
public class AdminLoginController extends BaseController {
	...
	@Autowired
	private AccountTeamService<AccountTeamModel> accountTeamService;//团队长账号
//
//	@Autowired
//	private AgentService<AgentModel> agentService;//代理账号
	...
	public String adminLogin(HttpServletRequest request, HttpServletResponse response, Account model) {
		...
		//团队长账号
		AccountTeamModel accountTeamModel = BeanUtils.copy(model, AccountTeamModel.class);
		accountTeamModel = accountTeamService.queryByCondition(accountTeamModel);
		if(accountTeamModel != null && accountTeamModel.getId() > ManagerConstant.PUBLIC_CONSTANT.SIZE_VALUE_ZERO){
			accountId = accountTeamModel.getId();
			adminAccount = BeanUtils.copy(accountTeamModel, Account.class);
		}
//		//代理账号
